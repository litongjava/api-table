...
function layerOpenForm(layer, title, content) {
  ...
  var width = (window.cols[0].length - 2) * 54;
...
function layuiSelectAddChildForObject(url, selectId, form) {
  $.ajax(url).then(function(resp) {
    if(resp.code >= 0) {
      var selectElement = document.getElementById(selectId); //select定义的id
      for(var p in resp.data) {
        var option = document.createElement("option"); // 创建添加option属性
        option.setAttribute("value", resp.data[p].id); // 给option的value添加值
        option.innerText = resp.data[p].name; // 打印option对应的纯文本 
        selectElement.appendChild(option); //给select添加option子标签
        if(form) {
          form.render('select');
        }
      }
